    ...
    <ion-content>
      <div class="ion-padding">
        ...
        <div class="phone-number-group">
          ...
          <ion-button
            :disabled="!clientSigninStore.isPhoneNumberValidated || clientSigninStore.isSending || clientSigninStore.isAuthing || myStore.isLoggingIn"
            expand="block"
            @click="() => {
        ...
        <div v-if="clientSigninStore.isSended" class="auth-number-group">
          ...
          <ion-button
            expand="block"
            :disabled="!clientSigninStore.isAuthNumberValidated || clientSigninStore.isSending || clientSigninStore.isAuthing || myStore.isLoggingIn"
            @click="() => { onClickStart() }"
          >
            동의하고 시작하기
            <ion-spinner v-if="clientSigninStore.isAuthing || myStore.isLoggingIn" class="ion-margin-start" />
          </ion-button>
        </div>
      </div>
    </ion-content>
...
const clientSigninStore = useWorkerSigninStore();
...
const onClickStart = async () => {
  ...
  if (clientSigninStore.isNewUser) {
    initTimer();
    router.push({ name: ROUTE_NAMES.SIGNUP_STEP1 });
    return;
  }
